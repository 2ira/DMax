...
class ModelRunner:
    ...
    def forward(
        self,
        input_ids: torch.Tensor = None,  # Will always be provided according to your setup
        position_ids: torch.Tensor = None,
        inputs_embeds: torch.Tensor = None,
        pp_proxy_tensors: Optional[torch.Tensor] = None,
        past_key_values=None,
        ...
        # 1. PRECEDENCE LOGIC: inputs_embeds takes priority
        if inputs_embeds is not None:
            active_input = inputs_embeds
            length = inputs_embeds.shape[1]
        elif input_ids is not None:
            active_input = input_ids
            length = input_ids.shape[1]
        else:
            raise ValueError("ModelRunner.forward requires either input_ids or inputs_embeds")

        # Decode phase is active if we are using cache and past_key_values exist
        is_decode_phase = use_cache == True and past_key_values is not None
        ...
        if can_run_graph and self.enable_cuda_graph:
            ...
            # 2. EMBEDDING CONVERSION FOR GRAPH
            # The CUDA graph ONLY accepts embeddings now.
            # If the user didn't provide inputs_embeds, we MUST generate them from input_ids.
            actual_embeds = inputs_embeds
            if actual_embeds is None:
                actual_embeds = self.model.model.word_embeddings(input_ids)

            ret = self.graph_runner.replay(
                inputs_embeds=actual_embeds,  # Always pass embeddings to the graph
                position_ids=position_ids,
                past_key_values=past_key_values,
                is_decode_phase=is_decode_phase,
                length=length,
...
class CudaGraphRunner:
    def __init__(self, model_runner: ModelRunner):
        ...
        with torch.device(self.device):
            ...
            # FIX 3: Add a static buffer for inputs_embeds
            hidden_size = self.model_runner.model.config.hidden_size
            self.inputs_embeds = torch.zeros((self.max_num_token, hidden_size), dtype=torch.bfloat16)
    ...
    def capture_one_batch_size(self, bs: int, forward: Callable, is_decode_phase:bool=True, length:int=0, cache_length:int=0, use_mask:bool=False):
        ...
        # Graph inputs
        inputs_embeds = self.inputs_embeds[:num_tokens].view(bs, num_tokens_per_bs, -1)
    ...
    def can_run(self, inputs, position_ids, past_key_values, is_decode_phase=True, length=0, cache_length=0):
        cuda_graph_bs = inputs.shape[0]
    ...
    def replay_prepare(self, inputs_embeds, position_ids, past_key_values, is_decode_phase, length, attention_mask, cache_length):
        raw_bs = inputs_embeds.shape[0]
        ...
        padded_num_token = bs * length

        # 拷贝真实数据到静态 buffer
        self.inputs_embeds[:raw_num_token].copy_(
            inputs_embeds.contiguous().view(-1, inputs_embeds.shape[-1])
        )
        if padded_num_token > raw_num_token:
            self.inputs_embeds[raw_num_token:padded_num_token].zero_()
        self.position_ids[:raw_num_token].copy_(position_ids.flatten())
        if padded_num_token > raw_num_token:
            self.position_ids[raw_num_token:padded_num_token].zero_()
        ...
        if is_decode_phase:
            ...
            if bs > raw_bs:
                self.past_key_values[:, :, raw_bs:bs, :, :minimal_length].zero_()
        if attention_mask is not None:
            ...
            if bs > raw_bs:
                self.attention_mask[raw_bs:bs, :length, :minimal_length].zero_()
        self.raw_bs = raw_bs
    ...
    def replay(
        self, inputs_embeds, position_ids, past_key_values, is_decode_phase, length, attention_mask, cache_length
    ):
        self.replay_prepare(inputs_embeds, position_ids, past_key_values, is_decode_phase, length, attention_mask, cache_length)
